...
public class MazeGenerator : MonoBehaviour
{
    ...
    private int[,] Maze;
    private List<Vector3> pathMazes = new();
    private Stack<Vector2> _tiletoTry = new();

    private List<Vector2> offsets = new()
        ...
        for (var i = 0; i <= Maze.GetUpperBound(0); i++)
        {
            for (var j = 0; j <= Maze.GetUpperBound(1); j++)
                if (Maze[i, j] == 1)
                {
                    ...
                    ptype.transform.position = new Vector3(i * ptype.transform.localScale.x, 0,
                        j * ptype.transform.localScale.z);
                    ptype.transform.localScale =
                        new Vector3(ptype.transform.localScale.x, 5, ptype.transform.localScale.z);
